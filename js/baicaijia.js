...
//     // }
// })
var baicaijiaController = myApp.controller('baicaijiaController', ['$scope', '$http', '$sce', function($scope, $http, $sce) {
    $scope.getBaicaijiatitle = function() {
        $http({
            method: 'get',
            url: 'http://mmb.ittun.com/api/getbaicaijiatitle'
        }).success(function(data) {
            $scope.baicaijiatitles = data.result;
        });
    }
    $scope.getBaicaijiatitle();
    $scope.getBaicaijiaProduct = function() {
        $http({
            method: 'get',
            url: 'http://mmb.ittun.com/api/getbaicaijiaproduct',
            params: { "titleid": $scope.titleid || 0 }
        }).success(function(data) {
            for (var i = 0; i < data.result.length; i++) {
                data.result[i].productName = $sce.trustAsHtml(data.result[i].productName);
                data.result[i].productPrice = $sce.trustAsHtml(data.result[i].productPrice);
                data.result[i].productImg = $sce.trustAsHtml(data.result[i].productImg);
                data.result[i].productCoupon = $sce.trustAsHtml(data.result[i].productCoupon);
                data.result[i].productHref = $sce.trustAsHtml(data.result[i].productHref);
                data.result[i].productCouponRemain = $sce.trustAsHtml(data.result[i].productCouponRemain);
            }
            $scope.baicaijiaproducts = data.result;
            $('.ul-wapper > .tabs > li').eq($scope.titleid || 0).addClass("active");
        });
    }
    $scope.getBaicaijiaProduct();
}]);
